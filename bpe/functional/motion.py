...
def npy2annotations(json_dir):
    recon2D = np.load(json_dir)['reconstruction']
    # Serialization (numpyData['annotations'][0]['objects'][0]['keypoints'])
    jointDict = {"annotations": []}
    for cnt in range(recon2D[0].shape[0]):
      #pdb.set_trace()
      new_array = np.concatenate((recon2D[0,cnt], np.ones((recon2D[0,cnt].shape[0],1))), axis=1).reshape(-1)
      jointDict['annotations'].append({"frame_num":cnt,"objects" : [{"keypoints" :new_array}]})
    return jointDict['annotations']
...
def cocopose2motion(num_joints, json_dir, scale=1.0, smooth=True, visibility=False, mean_height=None, limit=-1):
    ...
    annotations = npy2annotations(json_dir)
